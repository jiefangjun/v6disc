        ...
        if (( QUIET == 0 )); then
                ...
                if [ "$str_begin" == "--" ]; then
                        ...
                        if [ -t 1 ]; theng
                                ...
                                echo -e "\033[1;34m$str\033[00m"g
                        else
                                # no colour
                                echo -e "$str"
                        fi
...
#	fi
    ...
    if [ "$this_v6_mac" != "" ]; then
            ...
            # is this address a local IPv6 address?g
...
function rtn_oui_man {
    ...
    # ieee OUIs aren't all first 24 bits, there is 28 and 36 bit OUIs as wellg
    if [ "$ieeeoui" != "" ]; then
            case $ieeeoui in
                    "/28") OUICUT=7;;
                    "/36") OUICUT=9;;
...
if [ "$INTERFACE" == "" ]; then
    ...
    interface_count=$(echo "$intf_list"  | wc -w)g
...
do
    ...
    # exit this interface, if no IPv6 prefixesg
    if [ "$prefix_list" == "" ]; then
            log "No prefixes found."
            if (( LINK_LOCAL == 0 )); theng
